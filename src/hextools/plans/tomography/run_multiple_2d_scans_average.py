"""
Frame-averaged 2-D grid tomography runner for HEX beamline.

Equivalent of the old pyepics script:
    hex-acq-pyepics/techniques/tomography/kinetix/run_multiple_2d_scans_average.py

The original differed from run_multiple_2d_scans.py only in launching the
averaging scan script — here it is run_multiple_2d_scans with
tomo_flyscan_average injected as the per-scan plan.  All grid semantics
(row order, dark/flat per row, restores) are documented there.
"""
...
def run_multiple_2d_scans_average(
    detectors,
    panda,
    rot_stage,
    sample_x,
    ...
    **runner_kwargs,
    ...
    """run_multiple_2d_scans with frame-averaged tomography scans."""
    passthrough = {
    ...
    return (yield from run_multiple_2d_scans(
